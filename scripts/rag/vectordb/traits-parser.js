...
function extractFeaturesFromDescriptionKeywords(description) {
  ...
  // 🔒 嚴格：僅明確棕櫚關鍵字才標棕櫚，不因「扇形」「羽片」等泛用詞誤標（避免灌木/草本被當棕櫚）
  if (/棕櫚|棕櫚科|棕櫚屬|棕櫚幹|椰子類|棕櫚形/.test(text)) features.push('棕櫚');
  ...
  // 扇形葉+掌狀裂 才標棕櫚（棕竹等），單獨扇形葉不標
  if ((/扇形葉|扇狀葉|扇形裂片/.test(text) && /掌狀裂|掌狀深裂|掌狀複葉/.test(text)) && !features.includes('棕櫚')) features.push('棕櫚');
