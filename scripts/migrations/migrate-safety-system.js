...
async function migrateSafetySystem(conn) {
  await conn.execute(`
  ...
  const [settingsRows] = await conn.execute('SELECT id FROM safety_settings WHERE id = 1');
...
async function migrate() {
  ...
  try {
    ...
    await migrateSafetySystem(connection);
...
module.exports = { migrateSafetySystem };
